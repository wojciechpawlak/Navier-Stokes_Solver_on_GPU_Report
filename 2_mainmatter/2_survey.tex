...
%Even NVIDIA and Intel are hedging their bets with OpenCL though, with both vendors offering software hooks for their respective hardware. Alhough both Intel and nVidia have signed on to openCL and both of them technically support it, the performance benefits that they offer are pretty poor when compared to AMD's APUs and GPUs.

% The only downside to OpenCL is that, like OpenGL, because everyone gets to have their say in it's development, it literally gets "designed by committee" and as a result, you have everyone trying to introduce shit that's good for them into it, and block stuff that's good for others, even if it's not explicitly bad for them.
...
\subsubsection{CUDA SDK}

%http://developer.nvidia.com/opencl


\subsection{AMD APP SDK}
AMD APP (Accelerated Parallel Processing) SDK, formerly ATI Stream, is the OpenCL library for AMD graphics cards.
...
A lot of knowledge can be gathered just looking at other projects being implemented using given technology on given platform. 
